...
- **libmpv pins:** `scripts/libmpv.lock.json` pins each platform's LGPL build by URL and SHA256. A build Grid ships must be a never-overwritten prerelease in Grid's own repository that carries its complete corresponding source beside the binaries (`build-libmpv-linux.yml` and `build-libmpv-windows.yml` publish both), and `LICENSES/` must hold the notice of everything compiled into it — including statically linked dependencies. Never ship a binary whose exact source revisions are unknown. Windows releases stay off until FFmpeg `n7.1.5` is pinned for both platforms and the installer has passed a clean-machine check. Never delete a `libmpv-*` prerelease that a published Grid release pinned: it carries that release's LGPL source. Update `src-tauri/licenses/README.md` (source, notices, evidence) in the same change. `tauri.windows.conf.json` must keep every `lib/windows/*.dll` and `LICENSES/` beside `grid.exe` and repeat `bin/rqbit` in `externalBin` (platform configs replace arrays); CI's `package-windows` job checks the installed layout.
