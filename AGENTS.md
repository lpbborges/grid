...
- **Window transparency:** the main window is `transparent: true` because mpv renders behind the webview on Linux and Windows. `src/app.html` wraps the body in a `display: contents` div, so the layout's opaque root is `body > div > div` — one level deeper than it looks. Any new opaque ancestor above the player hides the video completely, and the failure looks exactly like broken compositing rather than a CSS bug.
...
- **Subtitle fetching:** External subtitle URLs and every redirect target must pass `is_allowed_subtitle_url` (the strem.io allowlist). The Rust rate limit is a token bucket whose burst (`SUBTITLE_RATE_LIMIT_BURST`) must hold two playbacks of the frontend caps (`MAX_EXTERNAL_SUBTITLE_FETCHES`, `MAX_TORRENT_SUBTITLE_FETCHES`), pinned by a Rust test; change them together.
- **TS/Rust parity:** Validators exist on both sides and must agree: `isValidInfoHash` ↔ `is_valid_info_hash`, `isValidFileIdx` ↔ `is_valid_file_idx`, and subtitle/video extension checks (case-insensitive on both sides). Limits must agree too: `MAX_EXTERNAL_SUBTITLE_FETCHES` ≤ `MAX_SUBTITLE_FILES` (pinned by a Rust test), or the native player drops every external subtitle. `MAX_NATIVE_SUBTITLE_FILES` (`useMpvBackend`) must equal `MAX_SUBTITLE_FILES` (also pinned by a Rust test).
...
- **libmpv pins:** `scripts/libmpv.lock.json` pins each platform's LGPL build by URL and SHA256. Only pin a prerelease published by `build-libmpv-linux.yml` / `build-libmpv-windows.yml` (it carries the licence texts and the source), and never delete or edit a `libmpv-*` prerelease that a published Grid release pinned: it carries that release's LGPL source. `tauri.windows.conf.json` must keep every `lib/windows/*.dll` and `LICENSES/` beside `grid.exe` and repeat `bin/rqbit` in `externalBin` (platform configs replace arrays); CI's `package-windows` job checks the installed layout.
